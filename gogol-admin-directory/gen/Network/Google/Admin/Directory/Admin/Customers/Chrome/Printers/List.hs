...
-- |
-- Module      : Network.Google.Admin.Directory.Admin.Customers.Chrome.Printers.List
...
instance
  ...
  where
  type
    Rs AdminCustomersChromePrintersList =
      ListPrintersResponse
  type
  ...
  requestClient AdminCustomersChromePrintersList {..} =
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy AdminCustomersChromePrintersListResource
          )
          Core.mempty
