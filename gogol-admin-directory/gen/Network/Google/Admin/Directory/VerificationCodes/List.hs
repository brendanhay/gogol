...
-- |
-- Module      : Network.Google.Admin.Directory.VerificationCodes.List
...
type DirectoryVerificationCodesListResource =
  "admin"
    ...
    Core.:> Core.Get '[Core.JSON] VerificationCodes
...
instance
  ...
  where
  type
    Rs DirectoryVerificationCodesList =
      VerificationCodes
  type
  ...
  requestClient DirectoryVerificationCodesList {..} =
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy DirectoryVerificationCodesListResource
          )
          Core.mempty
