...
-- | A resource alias for adsensehost.accounts.adunits.getAdCode which the
...
aagacHostCustomChannelId :: Lens' AccountsAdunitsGetAdCode' (Maybe Text)
aagacHostCustomChannelId
  = lens _aagacHostCustomChannelId
      (\ s a -> s{_aagacHostCustomChannelId = a})
