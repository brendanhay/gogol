...
-- |
-- Module      : Network.Google.AppsReseller.Reseller.Subscriptions.Activate
...
instance
  ...
  type
    Scopes ResellerSubscriptionsActivate =
  ...
  requestClient ResellerSubscriptionsActivate {..} =
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy ResellerSubscriptionsActivateResource
          )
          Core.mempty
