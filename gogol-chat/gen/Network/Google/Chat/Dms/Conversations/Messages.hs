...
-- |
-- Module      : Network.Google.Chat.Dms.Conversations.Messages
...
-- 'ChatDmsConversationsMessages' request conforms to.
type ChatDmsConversationsMessagesResource =
  "v1"
...
-- | Legacy path for creating message. Calling these will result in a BadRequest response.
--
-- /See:/ 'newChatDmsConversationsMessages' smart constructor.
data ChatDmsConversationsMessages = ChatDmsConversationsMessages
  { -- | V1 error format.
...
-- | Creates a value of 'ChatDmsConversationsMessages' with the minimum fields required to make a request.
newChatDmsConversationsMessages ::
  ...
  Message ->
  ChatDmsConversationsMessages
newChatDmsConversationsMessages parent payload =
  ChatDmsConversationsMessages
    { xgafv = Core.Nothing,
...
instance
  Core.GoogleRequest
    ChatDmsConversationsMessages
  where
  type Rs ChatDmsConversationsMessages = Message
  type Scopes ChatDmsConversationsMessages = '[]
  requestClient ChatDmsConversationsMessages {..} =
    go
      parent
      xgafv
      accessToken
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy ChatDmsConversationsMessagesResource
          )
          Core.mempty
