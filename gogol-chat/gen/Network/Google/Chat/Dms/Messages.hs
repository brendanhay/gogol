...
-- |
-- Module      : Network.Google.Chat.Dms.Messages
...
-- 'ChatDmsMessages' request conforms to.
type ChatDmsMessagesResource =
  "v1"
...
-- | Legacy path for creating message. Calling these will result in a BadRequest response.
--
-- /See:/ 'newChatDmsMessages' smart constructor.
data ChatDmsMessages = ChatDmsMessages
  { -- | V1 error format.
...
-- | Creates a value of 'ChatDmsMessages' with the minimum fields required to make a request.
newChatDmsMessages ::
  ...
  Message ->
  ChatDmsMessages
newChatDmsMessages parent payload =
  ChatDmsMessages
    { xgafv = Core.Nothing,
...
instance Core.GoogleRequest ChatDmsMessages where
  type Rs ChatDmsMessages = Message
  type Scopes ChatDmsMessages = '[]
  requestClient ChatDmsMessages {..} =
    go
      parent
      xgafv
      accessToken
    ...
    where
      go =
        Core.buildClient
          (Core.Proxy :: Core.Proxy ChatDmsMessagesResource)
          Core.mempty
