...
-- |
-- Module      : Network.Google.Chat.Rooms.Messages
...
-- 'ChatRoomsMessages' request conforms to.
type ChatRoomsMessagesResource =
  "v1"
...
-- | Legacy path for creating message. Calling these will result in a BadRequest response.
--
-- /See:/ 'newChatRoomsMessages' smart constructor.
data ChatRoomsMessages = ChatRoomsMessages
  { -- | V1 error format.
...
-- | Creates a value of 'ChatRoomsMessages' with the minimum fields required to make a request.
newChatRoomsMessages ::
  ...
  Message ->
  ChatRoomsMessages
newChatRoomsMessages parent payload =
  ChatRoomsMessages
    { xgafv = Core.Nothing,
...
instance Core.GoogleRequest ChatRoomsMessages where
  type Rs ChatRoomsMessages = Message
  type Scopes ChatRoomsMessages = '[]
  requestClient ChatRoomsMessages {..} =
    go
      parent
      xgafv
      accessToken
    ...
    where
      go =
        Core.buildClient
          (Core.Proxy :: Core.Proxy ChatRoomsMessagesResource)
          Core.mempty
