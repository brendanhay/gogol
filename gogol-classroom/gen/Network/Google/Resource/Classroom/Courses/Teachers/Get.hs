...
type CoursesTeachersGetAPI =
     "v1" :>
       "courses" :>
         Capture "courseId" Text :>
           "teachers" :>
             Capture "userId" Text :> Get '[JSON] Teacher
...
-- token.
ctgKey :: Lens' CoursesTeachersGet' (Maybe Text)
ctgKey = lens _ctgKey (\ s a -> s{_ctgKey = a})

-- | OAuth 2.0 token for the current user.
ctgOauthToken :: Lens' CoursesTeachersGet' (Maybe Text)
ctgOauthToken
  = lens _ctgOauthToken
...
ctgAlt :: Lens' CoursesTeachersGet' Text
...
instance GoogleRequest CoursesTeachersGet' where
        ...
        requestWithRoute r u CoursesTeachersGet{..}
          = go _ctgXgafv _ctgQuotaUser _ctgPrettyPrint
              _ctgUploadProtocol
              _ctgPp
              _ctgCourseId
              _ctgAccessToken
          ...
          where go
                  = clientWithRoute
                      (Proxy :: Proxy CoursesTeachersGetAPI)
                      r
                      u
