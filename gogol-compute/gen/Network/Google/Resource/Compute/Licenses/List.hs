...
-- debian-cloud or windows-cloud. Caution This resource is intended for use
...
data LicensesList =
  LicensesList'
    { _llReturnPartialSuccess :: !(Maybe Bool)
...
licensesList pLlProject_ =
  LicensesList'
    { _llReturnPartialSuccess = Nothing
    , _llOrderBy = Nothing
...
llReturnPartialSuccess :: Lens' LicensesList (Maybe Bool)
llReturnPartialSuccess
  = lens _llReturnPartialSuccess
      (\ s a -> s{_llReturnPartialSuccess = a})
...
instance GoogleRequest LicensesList where
        ...
        requestClient LicensesList'{..}
          = go _llProject _llReturnPartialSuccess _llOrderBy
              _llFilter
              _llPageToken
              (Just _llMaxResults)
              (Just AltJSON)
