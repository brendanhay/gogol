...
pmDefaultViewport :: Lens' PublishedMap (Maybe [Double])
pmDefaultViewport
...
pmContents :: Lens' PublishedMap (Maybe [Maybe MapItem])
pmContents
...
-- rectangular bounding box of the default viewport. The numbers represent
-- latitude and longitude in decimal degrees.
mapDefaultViewport :: Lens' Map (Maybe [Double])
mapDefaultViewport
...
-- | The contents of this Map.
mapContents :: Lens' Map (Maybe [Maybe MapItem])
mapContents
...
gjpCoordinates :: Lens' GeoJsonPoint (Maybe [Double])
gjpCoordinates
...
gjsonpCoordinates :: Lens' GeoJsonPolygon [[[Double]]]
gjsonpCoordinates
...
rcTags :: Lens' RasterCollection (Maybe [Text])
rcTags
...
-- table.
tabSchema :: Lens' Table (Maybe (Maybe Schema))
tabSchema
...
tabFiles :: Lens' Table [Maybe File]
tabFiles
...
-- information about tags can be found in the Tagging data article of the
-- Maps Engine help center.
tabTags :: Lens' Table (Maybe [Text])
tabTags
  = lens _tabTags (\ s a -> s{_tabTags = a}) .
      mapping (_Default . _Coerce)
