...
-- |
-- Module      : Network.Google.Mirror.Timeline.Attachments.Delete
...
newMirrorTimelineAttachmentsDelete attachmentId itemId =
  MirrorTimelineAttachmentsDelete {attachmentId = attachmentId, itemId = itemId}
  ...
  requestClient MirrorTimelineAttachmentsDelete {..} =
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy MirrorTimelineAttachmentsDeleteResource
          )
          Core.mempty
