...
-- | A resource alias for androidpublisher.purchases.subscriptions.refund which the
...
-- token.
pKey :: Lens' PurchasesSubscriptionsRefund' (Maybe Text)
pKey = lens _pKey (\ s a -> s{_pKey = a})

-- | OAuth 2.0 token for the current user.
pOauthToken :: Lens' PurchasesSubscriptionsRefund' (Maybe Text)
pOauthToken
...
pAlt :: Lens' PurchasesSubscriptionsRefund' Alt
