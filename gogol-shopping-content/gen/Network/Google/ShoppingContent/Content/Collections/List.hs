...
-- |
-- Module      : Network.Google.ShoppingContent.Content.Collections.List
...
instance Core.GoogleRequest ContentCollectionsList where
  type
    Rs ContentCollectionsList =
      ListCollectionsResponse
  type
    Scopes ContentCollectionsList =
  ...
  requestClient ContentCollectionsList {..} =
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy ContentCollectionsListResource
          )
          Core.mempty
