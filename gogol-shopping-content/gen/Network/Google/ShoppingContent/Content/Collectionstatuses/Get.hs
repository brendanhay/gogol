...
-- |
-- Module      : Network.Google.ShoppingContent.Content.Collectionstatuses.Get
...
instance
  ...
  where
  type
    Rs ContentCollectionstatusesGet =
      CollectionStatus
  type
    Scopes ContentCollectionstatusesGet =
  ...
  requestClient ContentCollectionstatusesGet {..} =
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy ContentCollectionstatusesGetResource
          )
          Core.mempty
