...
--
-- /See:/ <https://developers.google.com/storage/docs/json_api/ Cloud Storage API Reference> for @storage.defaultObjectAccessControls.update@.
module Network.Google.API.Storage.DefaultObjectAccessControls.Update
    (
    -- * REST Resource
...
-- | A resource alias for storage.defaultObjectAccessControls.update which the
...
-- token.
doacuKey :: Lens' DefaultObjectAccessControlsUpdate' (Maybe Text)
doacuKey = lens _doacuKey (\ s a -> s{_doacuKey = a})

-- | OAuth 2.0 token for the current user.
doacuOauthToken :: Lens' DefaultObjectAccessControlsUpdate' (Maybe Text)
doacuOauthToken
