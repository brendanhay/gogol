...
-- overrides metadata.
--
-- /See:/ <https://developers.google.com/storage/docs/json_api/ Cloud Storage API Reference> for @storage.objects.copy@.
module Network.Google.API.Storage.Objects.Copy
    (
    -- * REST Resource
      ObjectsCopyAPI
...
-- | A resource alias for storage.objects.copy which the
...
data ObjectsCopy' = ObjectsCopy'
    ...
    , _ocProjection                     :: !(Maybe ObjectsCopy'Projection)
...
ocProjection :: Lens' ObjectsCopy' (Maybe ObjectsCopy'Projection)
ocProjection
