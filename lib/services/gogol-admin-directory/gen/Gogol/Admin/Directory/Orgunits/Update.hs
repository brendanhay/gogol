...
instance Core.GoogleRequest DirectoryOrgunitsUpdate
         where
        type Rs DirectoryOrgunitsUpdate = OrgUnit
        type Scopes DirectoryOrgunitsUpdate =
             '[Admin'Directory'Orgunit]
        requestClient DirectoryOrgunitsUpdate{..}
          = go customerId orgUnitPath xgafv accessToken
              callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DirectoryOrgunitsUpdateResource)
                      Core.mempty
