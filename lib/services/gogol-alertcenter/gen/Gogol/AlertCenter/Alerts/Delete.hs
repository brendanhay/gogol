...
instance Core.GoogleRequest AlertCenterAlertsDelete
         where
        type Rs AlertCenterAlertsDelete = Empty
        type Scopes AlertCenterAlertsDelete =
             '["https://www.googleapis.com/auth/apps.alerts"]
        requestClient AlertCenterAlertsDelete{..}
          = go alertId xgafv accessToken callback customerId
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AlertCenterAlertsDeleteResource)
                      Core.mempty
