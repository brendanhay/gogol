...
instance Core.GoogleRequest AlertCenterAlertsList
         where
        type Rs AlertCenterAlertsList = ListAlertsResponse
        type Scopes AlertCenterAlertsList =
             '["https://www.googleapis.com/auth/apps.alerts"]
        requestClient AlertCenterAlertsList{..}
          = go xgafv accessToken callback customerId filter
              orderBy
              pageSize
              pageToken
              uploadType
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AlertCenterAlertsListResource)
                      Core.mempty
