...
instance Core.GoogleRequest AndroidPublisherEditsDetailsGet where
  type Core.Scopes AndroidPublisherEditsDetailsGet = '[Androidpublisher'FullControl]
  type Core.Config AndroidPublisherEditsDetailsGet = AndroidPublisherService
  type Core.Response AndroidPublisherEditsDetailsGet = AppDetails
  toRequest Core.Service {serviceRequest, serviceParams = AndroidPublisherParams {..}} AndroidPublisherEditsDetailsGet {..} =
    serviceRequest
      { Core.method = "GET",
        Core.path =
          Core.toRequestPath
