...
instance Core.GoogleRequest BooksNotificationGet
         where
        type Rs BooksNotificationGet = Notification
        type Scopes BooksNotificationGet =
             '["https://www.googleapis.com/auth/books"]
        requestClient BooksNotificationGet{..}
          = go (Core.Just notificationId) xgafv accessToken
              callback
              locale
              source
              uploadType
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy BooksNotificationGetResource)
                      Core.mempty
