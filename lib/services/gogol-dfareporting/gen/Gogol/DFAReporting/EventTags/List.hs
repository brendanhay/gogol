                         ...
                         Core.QueryParam "campaignId" Core.Int64 Core.:>
                           Core.QueryParam "definitionsOnly" Core.Bool Core.:>
                             Core.QueryParam "enabled" Core.Bool Core.:>
                               Core.QueryParams "eventTagTypes"
...
instance Core.GoogleRequest DFAReportingEventTagsList
         where
        type Rs DFAReportingEventTagsList =
             EventTagsListResponse
        type Scopes DFAReportingEventTagsList =
             '[Dfatrafficking'FullControl]
        requestClient DFAReportingEventTagsList{..}
          = go profileId xgafv accessToken adId advertiserId
              callback
              campaignId
              definitionsOnly
              enabled
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DFAReportingEventTagsListResource)
                      Core.mempty
