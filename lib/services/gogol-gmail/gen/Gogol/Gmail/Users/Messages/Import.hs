                       ...
                       Core.QueryParam "deleted" Core.Bool Core.:>
                         Core.QueryParam "internalDateSource"
                           ...
                           Core.QueryParam "neverMarkSpam" Core.Bool Core.:>
                             Core.QueryParam "processForCalendar" Core.Bool
                               Core.:>
...
newGmailUsersMessagesImport payload =
  GmailUsersMessagesImport
    ...
    , deleted = Core.False
    , internalDateSource = UsersMessagesImportInternalDateSource_DateHeader
...
instance Core.GoogleRequest GmailUsersMessagesImport
         where
        ...
        requestClient GmailUsersMessagesImport{..}
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy GmailUsersMessagesImportResource)
                      Core.mempty

instance Core.GoogleRequest
           (Core.MediaUpload GmailUsersMessagesImport)
         where
        type Rs (Core.MediaUpload GmailUsersMessagesImport) =
             Message
        type Scopes
               (Core.MediaUpload GmailUsersMessagesImport)
             = Core.Scopes GmailUsersMessagesImport
        requestClient
          (Core.MediaUpload GmailUsersMessagesImport{..} body)
          = go userId xgafv accessToken callback
              (Core.Just deleted)
              (Core.Just internalDateSource)
              (Core.Just neverMarkSpam)
