                               ...
                               Core.QueryParam "upload_protocol" Core.Text
                                 ...
                                 Core.QueryParam "alt" Core.AltJSON Core.:>
                                   Core.QueryParam "uploadType" Core.Multipart
                                     Core.:>
                                     Core.MultipartRelated '[Core.JSON] Message
...
newGmailUsersMessagesInsert payload =
  GmailUsersMessagesInsert
    ...
    , deleted = Core.False
    , internalDateSource = UsersMessagesInsertInternalDateSource_ReceivedTime
    , payload = payload
...
instance Core.GoogleRequest GmailUsersMessagesInsert
         where
        ...
        requestClient GmailUsersMessagesInsert{..}
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy GmailUsersMessagesInsertResource)
                      Core.mempty

instance Core.GoogleRequest
           (Core.MediaUpload GmailUsersMessagesInsert)
         where
        type Rs (Core.MediaUpload GmailUsersMessagesInsert) =
             Message
        type Scopes
               (Core.MediaUpload GmailUsersMessagesInsert)
             = Core.Scopes GmailUsersMessagesInsert
        requestClient
          (Core.MediaUpload GmailUsersMessagesInsert{..} body)
          = go userId xgafv accessToken callback
              (Core.Just deleted)
              (Core.Just internalDateSource)
              uploadType
