...
instance Core.GoogleRequest GmailUsersMessagesSend
         where
        type Rs GmailUsersMessagesSend = Message
        type Scopes GmailUsersMessagesSend =
             '[Gmail'FullControl,
               Gmail'Addons'Current'Action'Compose, Gmail'Compose,
        ...
        requestClient GmailUsersMessagesSend{..}
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy GmailUsersMessagesSendResource)
                      Core.mempty

instance Core.GoogleRequest
           (Core.MediaUpload GmailUsersMessagesSend)
         where
        type Rs (Core.MediaUpload GmailUsersMessagesSend) =
             Message
        type Scopes (Core.MediaUpload GmailUsersMessagesSend)
             = Core.Scopes GmailUsersMessagesSend
        requestClient
          (Core.MediaUpload GmailUsersMessagesSend{..} body)
          = go userId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
