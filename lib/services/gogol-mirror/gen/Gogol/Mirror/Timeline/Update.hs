...
instance Core.GoogleRequest MirrorTimelineUpdate
         where
        ...
        requestClient MirrorTimelineUpdate{..}
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy MirrorTimelineUpdateResource)
                      Core.mempty

instance Core.GoogleRequest
           (Core.MediaUpload MirrorTimelineUpdate)
         where
        type Rs (Core.MediaUpload MirrorTimelineUpdate) =
             TimelineItem
        type Scopes (Core.MediaUpload MirrorTimelineUpdate) =
             Core.Scopes MirrorTimelineUpdate
        requestClient
          (Core.MediaUpload MirrorTimelineUpdate{..} body)
