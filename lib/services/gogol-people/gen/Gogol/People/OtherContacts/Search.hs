...
instance Core.GoogleRequest PeopleOtherContactsSearch
         where
        type Rs PeopleOtherContactsSearch = SearchResponse
        type Scopes PeopleOtherContactsSearch =
             '[Contacts'Other'Readonly]
        requestClient PeopleOtherContactsSearch{..}
          = go xgafv accessToken callback pageSize query
              readMask
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy PeopleOtherContactsSearchResource)
                      Core.mempty
