           ...
           Core.QueryParam "query" Core.Text Core.:>
             Core.QueryParam "language" Core.Text Core.:>
               Core.QueryParam "maxResults" Core.Word32 Core.:>
                 Core.QueryParam "orderBy" ActivitiesSearchOrderBy
                   Core.:>
...
instance Core.GoogleRequest PlusActivitiesSearch
         where
        type Rs PlusActivitiesSearch = ActivityFeed
        type Scopes PlusActivitiesSearch =
             '[Plus'Login, Plus'Me]
        requestClient PlusActivitiesSearch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy PlusActivitiesSearchResource)
                      Core.mempty
