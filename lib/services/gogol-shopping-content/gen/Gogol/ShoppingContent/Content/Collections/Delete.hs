...
instance Core.GoogleRequest ContentCollectionsDelete
         where
        type Rs ContentCollectionsDelete = ()
        type Scopes ContentCollectionsDelete =
             '["https://www.googleapis.com/auth/content"]
        requestClient ContentCollectionsDelete{..}
          = go merchantId collectionId xgafv accessToken
              callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ContentCollectionsDeleteResource)
                      Core.mempty
