...
instance Core.GoogleRequest ContentDatafeedsUpdate
         where
        type Rs ContentDatafeedsUpdate = Datafeed
        type Scopes ContentDatafeedsUpdate =
             '[Content'FullControl]
        requestClient ContentDatafeedsUpdate{..}
          = go merchantId datafeedId xgafv accessToken callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ContentDatafeedsUpdateResource)
                      Core.mempty
