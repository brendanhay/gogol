...
type SpannerProjectsInstancesDatabasesSessionsExecuteBatchDmlResource =
  "v1"
    Core.:> Core.CaptureMode "session" "executeBatchDml" Core.Text
    ...
    Core.:> Core.ReqBody '[Core.JSON] ExecuteBatchDmlRequest
    Core.:> Core.Post '[Core.JSON] ExecuteBatchDmlResponse
...
instance
  ...
  where
  type
    Rs SpannerProjectsInstancesDatabasesSessionsExecuteBatchDml =
