...
type TagManagerAccountsContainersEnvironmentsReauthorizeResource =
  "tagmanager"
    Core.:> "v2"
    Core.:> Core.CaptureMode "path" "reauthorize" Core.Text
...
instance
  ...
  where
  type
    Rs TagManagerAccountsContainersEnvironmentsReauthorize =
      Environment
  type
    Scopes TagManagerAccountsContainersEnvironmentsReauthorize =
      '[Tagmanager'Publish]
  requestClient
    TagManagerAccountsContainersEnvironmentsReauthorize {..} =
      go
